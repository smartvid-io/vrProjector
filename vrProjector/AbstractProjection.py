...
class AbstractProjection:
    ...
    def reprojectToThisThreaded(self, sourceProjection, numThreads):
        ...
        angleList = [self.angular_position((float(i) / fx, float(j) / fy)) for i in range(self.imsize[0]) for j in
                     range(self.imsize[1])]
    ...
    def _pixel_value_bilinear_interpolated(self, angle):
        ...
        pixelR = self.bilinear_interpolation(0, 0, [(-1, -1, pixelA[0]), (-1, 1, pixelB[0]), (1, -1, pixelC[0]),
                                                    (1, 1, pixelD[0])])
        pixelG = self.bilinear_interpolation(0, 0, [(-1, -1, pixelA[1]), (-1, 1, pixelB[1]), (1, -1, pixelC[1]),
                                                    (1, 1, pixelD[1])])
        pixelB = self.bilinear_interpolation(0, 0, [(-1, -1, pixelA[2]), (-1, 1, pixelB[2]), (1, -1, pixelC[2]),
                                                    (1, 1, pixelD[2])])
